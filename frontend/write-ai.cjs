            ...
            <div className="relative">
              <GlassCard className="p-8">
                <div className="grid grid-cols-2 gap-4">
                  {[{ icon: Brain, label: 'Deep Learning', color: 'violet' }, { icon: Eye, label: 'Vision AI', color: 'blue' }, { icon: MessageSquare, label: 'NLP', color: 'cyan' }, { icon: BarChart3, label: 'Analytics', color: 'emerald' }].map((item, i) => (
                    <motion.div key={i} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.5 + i * 0.1 }} className={${bt}p-4 rounded-xl ${dl}{colorMap[item.color]?.bg || 'bg-slate-50'} text-center${bt}}>
                      <item.icon className={${bt}w-8 h-8 ${dl}{colorMap[item.color]?.text || 'text-slate-600'} mx-auto mb-2${bt}} />
                      <p className="text-sm font-semibold text-slate-700">{item.label}</p>
                    </motion.div>
                  ))}
                </div>
              </GlassCard>
              <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 3, repeat: Infinity }} className="absolute -top-6 -right-6 p-4 rounded-2xl bg-white shadow-lg shadow-violet-500/10 border border-violet-100">
                <Sparkles className="w-6 h-6 text-violet-600" />
              </motion.div>
              <motion.div animate={{ y: [0, 10, 0] }} transition={{ duration: 4, repeat: Infinity }} className="absolute -bottom-6 -left-6 p-4 rounded-2xl bg-white shadow-lg shadow-pink-500/10 border border-pink-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {capabilities.map((cap, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
                <GlassCard className={${bt}p-6 border-l-4 ${dl}{colorMap[cap.color]?.border || 'border-l-slate-500'} hover:shadow-md transition-all h-full${bt}}>
                  <div className={${bt}w-12 h-12 rounded-xl ${dl}{colorMap[cap.color]?.bg || 'bg-slate-50'} ${dl}{colorMap[cap.color]?.text || 'text-slate-600'} flex items-center justify-center mb-4${bt}}>
...
fs.writeFileSync(path.join(__dirname, 'src', 'pages', 'public', 'AIServices.jsx'), content, 'utf8');
const count = (fs.readFileSync(path.join(__dirname, 'src', 'pages', 'public', 'AIServices.jsx'), 'utf8').match(/export default/g) || []).length;
console.log('AIServices.jsx: ' + count + ' export(s)');
